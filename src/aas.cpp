...
{ // BFS
    queue<int> q;
    for (int u = 0; u < numTeachers; ++u) { // Traverse all the teach pair
        if (pairU[u] == NIL) { // If the teacher is not be paired
            dist[u] = 0;
            q.push(u);
        }
        else
    ...
    dist[NIL] = INF;
    while (!q.empty()) { // Update until the queue is empty
        int u = q.front();
        q.pop();
        if (dist[u] < dist[NIL])
            for (int v = 0; v < numCourses; ++v)
                if (adjMatrix[u][v] && dist[pairV[v]] == INF) { // If the course is not be paired and never be visited
...
bool AAS::DFS(int u)
{ // DFS
    if (u != NIL) { // Using DFS need the teacher is not be paired
        for (int v = 0; v < numCourses; ++v)
            if (adjMatrix[u][v] && dist[pairV[v]] == dist[u] + 1 && DFS(pairV[v])) { // Judge whether there is any a road
                pairV[v] = u;
                pairU[u] = v;
                return true;
            }
...
{ // Update the table
    ...
    for (int i = 0; i < numTeachers; ++i) { // Insert teachers
        for (int j = 0; j < numCourses; ++j) { // Insert Courses
...
{ // Read data from teach.txt file
    ...
    if (fileAgain.is_open()) { // Enter the file to paint the matrix
        string line;
        int teacherIndex = 0;
        while (getline(fileAgain, line)) { // Read line by line to populate adjMatrix
            QStringList parts = QString::fromStdString(line).split(" ", QString::SkipEmptyParts);
            if (parts.size() > 1) {
                for (int i = 1; i < parts.size(); ++i) {
                    QString courseName = parts[i];
...
void AAS::on_startButton_clicked()
{
    if (!isOpened) {
        ...
        for (int u = 0; u < numTeachers; ++u) { // Load the Model
            if (pairU[u] == NIL)
        ...
        QMessageBox::information(this, "Error", "The table is already opened! You may reboot to load a new table");
}
